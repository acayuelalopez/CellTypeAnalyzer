...
public class StartPageModified extends AbstractWizardPage {
	...
	private JPanel histogramPanel, chartPanel2, controlPanel2;
	...
	public StartPageModified() {
		...
		JLabel imageCal = new JLabel("  Calibration Settings :   ");
		...
		JLabel channelLabelSet = new JLabel("  Channel Settings :   ");
		channelLabelSet.setBorder(BorderFactory.createTitledBorder(""));
		channelLabelSet.setFont(new Font(Font.DIALOG, Font.BOLD, 11));
		JLabel channe1 = new JLabel("          ▪  Cell-Detection Ch :   ");
		JLabel channe2 = new JLabel("          ▪  Customized Analysis Ch1 :   ");
		JLabel channe3 = new JLabel("          ▪  Customized Analysis Ch2 :   ");
		...
		JLabel cellLabel = new JLabel(" Nucleus-Characterization Settings ");
		...
		JLabel thresholdlLabel = new JLabel("AutoThreshold Method : ");
		...
		buttonOpenImage.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (imp != null) {
					...
					channelLabel.setText(" Channel-(w1) : " + tfCh1.getText() + " ");
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int r = 0; r < data.length; r++)
					for (int c = 0; c < data[r].length; c++)
						values[r] = data[r][selectedIndex + 1];
	...
	public void preview1Tool() {
		...
		if (ScriptCustomizable_.text2 != null) {
			StringTokenizer st = new StringTokenizer(ScriptCustomizable_.text2, "\n");
			...
			for (int i = 0; i < n; i++) {
				...
				if (line.contains("run"))
					line = line.replaceAll("run", "").replaceAll("\\(", "").replaceAll("\\);", "").replaceAll("\"", "");
				if (line.contains("IJ.run"))
					line = line.replaceAll("IJ.run", "").replaceAll("\\(", "").replaceAll("\\);", "").replaceAll("\"", "");

				String[] args = line.split(",");
				for (int j = 1; j < args.length; j++)
					sb.append(args[j]);
				String argDef = new String(sb);
				IJ.log(args[0] + "------" + argDef);
				IJ.run(channelAnal, args[0], argDef);
			}
		}
		IJ.run(channelAnal, "Analyze Particles...", "size=50-2147483647 exclude clear add");
		roiManager.runCommand(channels[comboCh1.getSelectedIndex()], "Measure");
		...
		table.getColumnModel().getColumn(1).setHeaderValue("Cell-Label");
	...
	public void nextTool() {
		...
		if (imp == null) {
			...
			MiddlePageModified.channelLabel.setText(" Channel-(w2) : " + tfCh2.getText() + " ");
			...
			EndPageModified.channelLabel.setText(" Channel-(w3) : " + tfCh3.getText() + "  ");
			...
			MiddlePageModified2.scatLabel
					.setText(" Scatter Plot for " + tfCh2.getText() + " - " + tfCh3.getText() + " Channels :   ");

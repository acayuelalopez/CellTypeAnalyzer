...
public class ExportActionBatchMode {
	...
	public void exportExcelXlsActionPerformed(java.awt.event.ActionEvent evt) throws IOException {
		...
		/*
		 * TableModel model1 = table1.getModel(); TableModel modelS1 =
		 * tableS1.getModel(); FileWriter out1 = new FileWriter(new
		 * File(directFolder.getAbsolutePath() + File.separator + "TableData_w2" + ".xls")); for
		 * (int i = 0; i < model1.getColumnCount(); i++) {
		 * out1.write(model1.getColumnName(i) + "\t"); } out1.write("\n");
		 * 
		 * for (int i = 0; i < table1.getRowCount(); i++) { for (int j = 0; j <
		 * model1.getColumnCount(); j++) { IJ.log(i+"----"+j+"------");
		 * out1.write(model1.getValueAt(i, j).toString() + "\t"); } out1.write("\n"); }
		 * for (int i = 0; i < modelS1.getColumnCount(); i++) {
		 * out1.write(modelS1.getColumnName(i) + "\t"); } out1.write("\n");
		 * 
		 * for (int i = 0; i < modelS1.getRowCount(); i++) { for (int j = 0; j <
		 * modelS1.getColumnCount(); j++) { out1.write(modelS1.getValueAt(i,
		 * j).toString() + "\t"); } out1.write("\n"); }
		 * 
		 * out1.close();
		 */
		// 2
		/*
		 * TableModel model2 = table2.getModel(); TableModel modelS2 =
		 * tableS2.getModel(); FileWriter out2 = new FileWriter(new
		 * File(directFolder.getAbsolutePath() + File.separator + "TableData_w3" + ".xls")); for
		 * (int i = 0; i < model2.getColumnCount(); i++) {
		 * out2.write(model2.getColumnName(i) + "\t"); } out2.write("\n");
		 * 
		 * for (int i = 0; i < model2.getRowCount(); i++) { for (int j = 0; j <
		 * model2.getColumnCount(); j++) { out2.write(model2.getValueAt(i, j).toString()
		 * + "\t"); } out2.write("\n"); } for (int i = 0; i < modelS2.getColumnCount();
		 * i++) { out2.write(modelS2.getColumnName(i) + "\t"); } out2.write("\n");
		 * 
		 * for (int i = 0; i < modelS2.getRowCount(); i++) { for (int j = 0; j <
		 * modelS2.getColumnCount(); j++) { out2.write(modelS2.getValueAt(i,
		 * j).toString() + "\t"); } out2.write("\n"); }
		 * 
		 * out2.close();
		 */

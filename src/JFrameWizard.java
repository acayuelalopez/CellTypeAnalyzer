...
public class JFrameWizard extends JFrame implements Wizard {
	...
	private int classCounterDef, classCounterDef2;
	...
	public void BatchModeGUI() {
		...
		panelDefault.setLayout(new FlowLayout(0));
		panelSave.setLayout(new FlowLayout(0));
		panelExport.setLayout(new FlowLayout(0));
		JLabel defaultAnal = new JLabel("     User-Default Analysis");
		JLabel directXml = new JLabel("     .XML Analysis :   ");
		JRadioButton xmlCheck = new JRadioButton("");
		JRadioButton defaultAnalysis = new JRadioButton("");
		ButtonGroup bgroup2 = new ButtonGroup();
		bgroup2.add(xmlCheck);
		bgroup2.add(defaultAnalysis);
		JLabel directSave = new JLabel("         Directoy to Export Results :   ");
		JLabel fileToExport = new JLabel("         File Format to export results :   ");
		defaultAnal.setFont(new Font("Helvetica", 1, 12));
		defaultAnalysis.setSelected(true);
		defaultAnal.setEnabled(true);
		...
		defaultAnalysis.addItemListener(new ItemListener() {
		...
		/*
		 * okButtonClass.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { selectedRow =
		 * ColorEditorEnd.tableC.getSelectedRow(); parameters = ((JLabel)
		 * ColorEditorEnd.tableC.getModel().getValueAt(
		 * ColorEditorEnd.tableC.convertRowIndexToModel(selectedRow),
		 * ColorEditorEnd.tableC.convertColumnIndexToModel(2))).getText().toString().
		 * replace("<html>", ""); className = ((JLabel)
		 * ColorEditorEnd.tableC.getModel().getValueAt(
		 * ColorEditorEnd.tableC.convertRowIndexToModel(selectedRow),
		 * ColorEditorEnd.tableC.convertColumnIndexToModel(0))).getText().toString();
		 * frameClass.dispatchEvent(new WindowEvent(frameClass,
		 * WindowEvent.WINDOW_CLOSING)); } });
		 * 
		 * cancelButtonClass.addItemListener(new ItemListener() { public void
		 * itemStateChanged(ItemEvent e) { frameClass.dispatchEvent(new
		 * WindowEvent(frameClass, WindowEvent.WINDOW_CLOSING));
		 * 
		 * } });
		 */
			...
			public void actionPerformed(ActionEvent evt) {
				...
				Thread mainProcess = new Thread(new Runnable() {
					public void run() {
						...
						if (defaultAnalysis.isSelected() == Boolean.TRUE) {
							
							DefaultAnalysis_ da = new DefaultAnalysis_();
							da.run(textSave.getText(), StartPageModified.textImages.getText(), listOfFiles);
						}
						if (defaultAnalysis.isSelected() == Boolean.FALSE) {
							...
							for (int ix = 0; ix < listOfFiles.length; ++ix) {
								...
								channelAnalCellCount = channels[StartPageModified.comboCh1.getSelectedIndex()];
								ImagePlus impSeg = channelAnalCellCount.duplicate();
								if (StartPageModified.selectedItem.equals("Default") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Default white");
								if (StartPageModified.selectedItem.equals("Huang") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Huang white");
								if (StartPageModified.selectedItem.equals("Huang2") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Huang2 white");
								if (StartPageModified.selectedItem.equals("Intermodes") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Intermodes white");
								if (StartPageModified.selectedItem.equals("Li") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Li white");
								if (StartPageModified.selectedItem.equals("MaxEntropy") == true)
									IJ.run(impSeg, "Auto Threshold", "method=MaxEntropy white");
								if (StartPageModified.selectedItem.equals("Mean") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Mean white");
								if (StartPageModified.selectedItem.equals("MinError(I)") == true)
									IJ.run(impSeg, "Auto Threshold", "method=MinError(I) white");
								if (StartPageModified.selectedItem.equals("Minimum") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Minimum white");
								if (StartPageModified.selectedItem.equals("Otsu") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Otsu white");
								if (StartPageModified.selectedItem.equals("RenyiEntropy") == true)
									IJ.run(impSeg, "Auto Threshold", "method=RenyiEntropy white");
								if (StartPageModified.selectedItem.equals("Shanbhag") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Shanbhag white");
								if (StartPageModified.selectedItem.equals("Triangle") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Triangle white");
								if (StartPageModified.selectedItem.equals("Yen") == true)
									IJ.run(impSeg, "Auto Threshold", "method=Yen white");

								if (rm != null)
									rm.reset();

								IJ.run(impSeg, "Analyze Particles...", "size=50-2147483647 exclude clear add");
								...
								channelAnalysis1 = channels[StartPageModified.comboCh3.getSelectedIndex()];
								...
								for (jxxxxxx = 0; jxxxxxx < dataList1.size(); ++jxxxxxx) {
									for (jxxxxxxxxxxx = 1; jxxxxxxxxxxx < ((List) dataList1.get(jxxxxxx))
											.size(); ++jxxxxxxxxxxx) {
								...
								for (int p = 0; p < counterFM.length; p++)
									rt1.setValue("FociNuc", p, counterFM[p]);
								...
								List<String> mainParameters1 = new ArrayList<String>();
								List<Double> minValue1 = new ArrayList<Double>();
								List<Double> maxValue1 = new ArrayList<Double>();
								for (int i = 0; i < splitParameter.length; i++)
									if (splitParameter[i].contains(StartPageModified.tfCh2.getText()) == Boolean.TRUE) {

										mainParameters1.add(splitParameter[i]
												.substring(0, splitParameter[i].indexOf(":")).replace("<br>", ""));
										minValue1.add(Double.parseDouble(splitParameter[i].substring(
												splitParameter[i].indexOf("[") + 1, splitParameter[i].indexOf(","))));
										maxValue1.add(Double.parseDouble(splitParameter[i].substring(
												splitParameter[i].indexOf(",") + 1, splitParameter[i].indexOf("]"))));

									}

								for (int u = 0; u < model1.getRowCount(); ++u)
									for (int z = 0; z < mainParameters1.size(); z++) {

										if ((Double) table1.getModel().getValueAt(u,
												table1.getColumn(mainParameters1.get(z)).getModelIndex()) >= (minValue1
														.get(z))
												&& (Double) table1.getModel().getValueAt(u, table1
														.getColumn(mainParameters1.get(z)).getModelIndex()) <= maxValue1
																.get(z)) {
											model1.setValueAt(className, u, table1.convertColumnIndexToModel(0));
											rt1.setValue("Class-Label", u, className);

										}

										for (jxxxxxxxxxxxx = 0; jxxxxxxxxxxxx < table1.getModel()
								...
								channelAnalysis2 = channels[StartPageModified.comboCh2.getSelectedIndex()];
								...
								int ux;
								int c;
								List<String> mainParameters2 = new ArrayList<String>();
								List<Double> minValue2 = new ArrayList<Double>();
								List<Double> maxValue2 = new ArrayList<Double>();
								for (int i = 0; i < splitParameter.length; i++)
									if (splitParameter[i].contains(StartPageModified.tfCh3.getText()) == Boolean.TRUE) {

										mainParameters2.add(splitParameter[i]
												.substring(0, splitParameter[i].indexOf(":")).replace("<br>", ""));
										minValue2.add(Double.parseDouble(splitParameter[i].substring(
												splitParameter[i].indexOf("[") + 1, splitParameter[i].indexOf(","))));
										maxValue2.add(Double.parseDouble(splitParameter[i].substring(
												splitParameter[i].indexOf(",") + 1, splitParameter[i].indexOf("]"))));

									}

								for (int u = 0; u < model2.getRowCount(); ++u)
									for (int z = 0; z < mainParameters2.size(); z++) {

										if ((Double) table2.getModel().getValueAt(u,
												table2.getColumn(mainParameters2.get(z)).getModelIndex()) >= (minValue2
														.get(z))
												&& (Double) table2.getModel().getValueAt(u, table2
														.getColumn(mainParameters2.get(z)).getModelIndex()) <= maxValue2
																.get(z)) {
											model2.setValueAt(className, u, table2.convertColumnIndexToModel(0));
											rt2.setValue("Class-Label", u, className);
										}
									}
								...
								for (int rowx = 0; rowx < dataPositiveDetection.length; ++rowx) {
									if (dataPositiveDetection[rowx][0] == className) {
										counter1Def = counter1++;
									}

									if (dataPositiveDetection[rowx][1] == className) {
										counter2Def = counter2++;
									}

									if (dataPositiveDetection[rowx][0] == className
											&& dataPositiveDetection[rowx][1] == className) {
							...
							Object[] columHeadersFinal = new Object[] { "Image-Name",
									StartPageModified.tfCh1.getText() + "-Count",
									StartPageModified.tfCh1.getText() + "-%",
									StartPageModified.tfCh2.getText() + "-Count",
									StartPageModified.tfCh2.getText() + "-%",
									StartPageModified.tfCh3.getText() + "-Count",
									StartPageModified.tfCh3.getText() + "-%", "+ -Count", "+ -%" };
						...
						if (xmlCheck.isSelected() == Boolean.TRUE) {
							...
							try {
								...
								File file = new File(textXml.getText());
							...
							for (int ix = 0; ix < listOfFiles.length; ++ix) {
								...
								for (col = 1; col < tableData[col].length; ++col) {
									double sum = 0.0D;
									...
									listMaxValue.add(sum / (double) tableData.length);
								}
	...
	public void writerCsvExcel(JTable table0, JTable table1, JTable table2, String location) {

		HSSFWorkbook fWorkbook = new HSSFWorkbook();

		////////////////// Dapi
		HSSFCellStyle cellStyle0 = fWorkbook.createCellStyle();
		HSSFFont sheetTitleFont0 = fWorkbook.createFont();
		sheetTitleFont0.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);
		HSSFSheet fSheet0 = fWorkbook.createSheet("DAPI -(w1)");
		TableModel model0 = table0.getModel();

		// Get Header
		TableColumnModel tcm0 = table0.getColumnModel();
		HSSFRow hRow0 = fSheet0.createRow((short) 0);
		for (int j = 0; j < tcm0.getColumnCount(); j++) {
			HSSFCell cell0 = hRow0.createCell((short) j);
		...
		for (int i = 0; i < model0.getRowCount(); i++) {
			...
			for (int j = 0; j < model0.getColumnCount(); j++) {
				HSSFCell cell0 = fRow0.createCell((short) j);
				cell0.setCellValue(model0.getValueAt(i, j).toString());
				cell0.setCellStyle(cellStyle0);
			}
		}
		////////////////// h2Ax
		HSSFCellStyle cellStyle1 = fWorkbook.createCellStyle();
		HSSFFont sheetTitleFont1 = fWorkbook.createFont();
		HSSFSheet fSheet1 = fWorkbook.createSheet("h2Ax -(w3)");
		TableModel model1 = table1.getModel();

		// Get Header
		TableColumnModel tcm1 = table1.getColumnModel();
		HSSFRow hRow1 = fSheet1.createRow((short) 0);
		for (int j = 0; j < tcm1.getColumnCount(); j++) {
			HSSFCell cell1 = hRow1.createCell((short) j);
		...
		for (int i = 0; i < data1.length; i++) {
			...
			for (int j = 0; j < data1[i].length; j++) {
				HSSFCell cell1 = fRow1.createCell((short) j);
				cell1.setCellValue(data1[i][j].toString());
				cell1.setCellStyle(cellStyle1);
			}
		}
//////////////////p21
		HSSFCellStyle cellStyle2 = fWorkbook.createCellStyle();
		HSSFFont sheetTitleFont2 = fWorkbook.createFont();
		HSSFSheet fSheet2 = fWorkbook.createSheet("p21 -(w2)");
...
// Get Header
		...
		for (int j = 0; j < tcm2.getColumnCount(); j++) {
			HSSFCell cell2 = hRow2.createCell((short) j);
		...
		for (int i = 0; i < data2.length; i++) {
			...
			for (int j = 0; j < data2[i].length; j++) {
				HSSFCell cell1 = fRow1.createCell((short) j);
				cell1.setCellValue(data2[i][j].toString());
				cell1.setCellStyle(cellStyle1);
			}
		}
		try {
